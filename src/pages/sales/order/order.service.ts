...
export class OrderService {
  ...
  async addOrder(addOrderDto: AddOrderDto): Promise<ResponsePayload> {
    ...
    try {
      ...
      if (addOrderDto.user) {
        ...
        const rewardPointData: Reward = await this.rewardModel.findOne({});
          ...
          { _id: addOrderDto.user },
          {
            $inc: {
              userPoints: Math.floor(
                (rewardPointData.rewardPoint * subTotal) / 100,
              ),
            },
          },
  ...
  async updateOrderPaymentIdById(
    id: string,
    updateOrderPaymentDto: UpdateOrderPaymentDto,
  ): Promise<ResponsePayload> {
    try {
      await this.orderModel.findByIdAndUpdate(id, {
        $set: updateOrderPaymentDto,
      });
      return {
        success: true,
        message: 'Order updated successfully',
