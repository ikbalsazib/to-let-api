...
export class PromoOfferService {
  ...
  async getAllPromoOffersUI(
    filterPromoOfferDto: FilterAndPaginationPromoOfferDto,
    searchQuery?: string,
  ): Promise<ResponsePayload> {
    const { paginationProduct } = filterPromoOfferDto;
    const { sort } = filterPromoOfferDto;
    try {
      const data = await this.promoOfferModel
        .find(
          { status: 'publish' },
          {
            products: {
              $slice: [
                paginationProduct.currentPage * paginationProduct.pageSize,
                paginationProduct.pageSize,
              ],
            },
          },
        )
        .populate({
          ...
          model: 'Product',
          select:
            'name nameBn slug images category subCategory brand costPrice salePrice discountType discountAmount hasVariations status videoUrl serial',
        });
