...
export class ProductService {
  ...
  async addProductByUser(
    ...
    try {
      ...
      const mAddProductDto = { ...addProductDto, ...{ user: userData } };

      return this.addProduct(mAddProductDto);
    } catch (error) {
      console.log(error);
      if (error.code && error.code.toString() === ErrorCodes.UNIQUE_FIELD) {
        throw new ConflictException('Slug Must be Unique');
  ...
  async getAllProducts(
    ...
    if (filter && filter['childCategory._id']) {
      filter['childCategory._id'] = new ObjectId(filter['childCategory._id']);
    }
      ...
      if (filterGroup.category) {
        groupCategory = {
          $group: {
            ...
            nameBn: { $first: '$category.nameBn' },
            nameIt: { $first: '$category.nameIt' },
      ...
      if (filterGroup.brand) {
        groupBrand = {
          $group: {
            ...
            nameBn: { $first: '$brand.nameBn' },
            nameIt: { $first: '$brand.nameIt' },
      ...
      if (filterGroup.subCategory) {
        groupSubCategory = {
          $group: {
            ...
            nameBn: { $first: '$subCategory.nameBn' },
            nameIt: { $first: '$subCategory.nameIt' },
